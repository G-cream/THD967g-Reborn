...
function DestructableCounter takes nothing returns nothing
	local integer count = LoadInteger(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("DestructableCount"))
	call SaveInteger(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("DestructableCount"), count + 1)
endfunction
function RepelTargetTimerProc takes nothing returns nothing
	local timer t = GetExpiredTimer()
	local unit target = LoadUnitHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Target"))
	local real repelAngle = LoadReal(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Angle"))
	local real repelMaxDistance = LoadReal(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("MaxDistance"))
	local real repelDistance = 9.0
	local real unitRepelPathWidth = 45.0
	local real destructableRepelPathWidth = 100.0
	local location repelStartPoint = LoadLocationHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("StartPoint"))
	local location repelCurrentPoint = LoadLocationHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("CurrentPoint"))
	...
	set destructableCount = LoadInteger(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("DestructableCount"))
	...
	if repelEndFlag == false then
		...
		call SaveLocationHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("CurrentPoint"), repelNextPoint)
	else
...
function RepelTarget takes nothing returns nothing
	local timer t = CreateTimer()
	local unit caster = LoadUnitHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Caster"))
	local unit target = LoadUnitHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Target"))
...
function Trig_ReisenNo201SpellEffect_Actions takes nothing returns nothing
	...
	call SaveUnitHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Caster"), caster)
	call SaveUnitHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Target"), target)
	call SaveReal(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("Angle"), repelAngle)
	call SaveReal(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("MaxDistance"), repelMaxDistance)
	call SaveLocationHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("StartPoint"), repelStartPoint)
	call SaveLocationHandle(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("CurrentPoint"), repelCurrentPoint)
	call SaveInteger(udg_ht, StringHash("ReisenNo201SpellEffectTrigger"), StringHash("DestructableCount"), 0)
...
function Trig_ReisenNo202SpellEffect_Actions takes nothing returns nothing
	...
	call SaveBoolean(udg_ht, StringHash("ReisenNo202SpellEffectTrigger"), StringHash("TriggeringFlag"), true)
	if UnitHasItemOfTypeBJ(caster,'I00B') then
...
function RemoveTargetDamage takes nothing returns nothing
	local unit target = LoadUnitHandle(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("Target"))
	local integer removeTimes = 0
	if HaveSavedInteger(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("RemoveTimes")) then
		set removeTimes = LoadInteger(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("RemoveTimes"))
	endif
	if removeTimes == 0 then
		call SaveReal(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("CurrentLife"), GetUnitState(target, UNIT_STATE_LIFE))
		call SetUnitState(target, UNIT_STATE_LIFE, GetUnitState(target, UNIT_STATE_MAX_LIFE))
	endif
	call SaveInteger(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("RemoveTimes"), removeTimes + 1)
	set target = null
endfunction
function TargetReceiveDamageTimerProc takes nothing returns nothing
	local unit target = LoadUnitHandle(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("Target"))
	local timer t = GetExpiredTimer()
	local real currentLife = LoadReal(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("CurrentLife"))
	local integer removeTimes = LoadInteger(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("RemoveTimes"))
	set removeTimes = removeTimes - 1
	call SaveInteger(udg_ht, StringHash("ReisenNo202AttackedTrigger"), StringHash("RemoveTimes"), removeTimes)
	if GetWidgetLife(target) > 0.405 and removeTimes == 0 then
...
function Trig_ReisenNo203Kill_Actions takes nothing returns nothing
...
function Trig_ReisenNo203Damage_Actions takes nothing returns nothing
	...
	if HaveSavedReal(udg_ht, StringHash("ReisenNo203KillTrigger"), StringHash("ExtraDamage")) then
		set extraDamage = LoadReal(udg_ht, StringHash("ReisenNo203KillTrigger"), StringHash("ExtraDamage"))
	endif
	if GetTriggerEventId() == EVENT_UNIT_DAMAGED and GetEventDamageSource() == caster then
		if abilityLevel >= 1 then
			...
			call SaveReal(udg_ht, StringHash("ReisenNo203KillTrigger"), StringHash("ExtraDamage"), 0.0)
...
function Trig_ReisenNo203Attack_Conditions takes nothing returns boolean
...
function Trig_ReisenNo203Attack_Actions takes nothing returns nothing
	local unit caster = GetAttacker()
	local unit target = GetTriggerUnit()
	local trigger reisenNo203DamageTrigger = CreateTrigger()
	local triggeraction reisenNo203DamageTriggerAction = TriggerAddAction(reisenNo203DamageTrigger, function Trig_ReisenNo203Damage_Actions)
...
function Trig_Initialing_ReisenNo2_Actions takes nothing returns nothing
	...
	call TriggerAddAction(reisenNo202SpellEffectTrigger, function Trig_ReisenNo202SpellEffect_Actions)
	...
	call TriggerAddCondition(reisenNo203AttackTrigger, Condition(function Trig_ReisenNo203Attack_Conditions))
	call TriggerAddAction(reisenNo203AttackTrigger, function Trig_ReisenNo203Attack_Actions)
	...
	call TriggerAddCondition(reisenNo203KillTrigger, Condition(function Trig_ReisenNo203Kill_Conditions))
	call TriggerAddAction(reisenNo203KillTrigger, function Trig_ReisenNo203Kill_Actions)
